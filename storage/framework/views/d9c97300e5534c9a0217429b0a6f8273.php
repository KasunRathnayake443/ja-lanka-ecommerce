...
<div class="bg-white border-b border-gray-100 py-10">
    <div class="max-w-[1400px] mx-auto px-5 md:px-10">
        <div class="flex flex-wrap items-end justify-between gap-4">
            <div>
                <div class="text-[0.72rem] font-semibold tracking-[0.14em] uppercase text-red-700 mb-1">Global Flavors Mart</div>
                <h1 class="font-['Cormorant_Garamond'] text-3xl md:text-4xl font-light text-gray-900">
                    All <em class="italic text-red-700 not-italic">Products</em>
                </h1>
            </div>
        ...
        <div>
            <div class="flex flex-wrap items-center justify-between gap-4 mb-7">
                <div class="font-['Cormorant_Garamond'] text-2xl font-light text-gray-900">
                    The <em class="italic text-red-700 not-italic">Collection</em>
                </div>
                <div id="topbarCount" class="text-xs text-gray-400">Loading products…</div>
            </div>
...
async function loadProducts() {
  ...
  try {
    ...
    console.error(e);
    document.getElementById('productsGrid').innerHTML = `<div class="col-span-full text-center py-20"><p class="text-sm text-red-600">Failed to load products. Please refresh.</p></div>`;
  } finally {
    isLoading = false;
...
function renderProducts(products, pageToLoad) {
  ...
  if (!products.length && pageToLoad === 1) {
    grid.innerHTML = `<div class="col-span-full text-center py-20"><p class="text-sm text-gray-400">No products match your filters.</p></div>`;
    return;
  }
  ...
  products.forEach(p => {
    ...
    const priceHtml = hasSale
      ? `<span class="product-price--sale text-base font-bold text-red-700">LKR ${parseFloat(p.sale_price).toLocaleString()}</span>
         <span class="product-price--was text-xs text-gray-400 line-through ml-2">LKR ${parseFloat(p.regular_price).toLocaleString()}</span>`
      : `<span class="product-price--reg text-base font-bold text-gray-900">LKR ${parseFloat(p.regular_price).toLocaleString()}</span>`;
...
<?php echo $__env->make('layouts.desktop', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH C:\Users\Kasun Rathnayake\Herd\ja-lanka-ecommerce\resources\views/product/shop.blade.php ENDPATH**/ ?>