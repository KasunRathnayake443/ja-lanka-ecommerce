<div class="hero relative w-full h-screen overflow-hidden bg-[#080808]"
    ...
    <div class="absolute bottom-0 left-0 w-full h-0.5 bg-white/10 z-30">
    ...
    <div class="absolute bottom-6 left-12 z-30 items-center gap-5 hidden lg:flex">
        <div class="text-[11px] tracking-[0.13em] text-white/35 font-mono whitespace-nowrap" x-text="counter"></div>
        <div class="flex gap-1">
            <button @click="prev" aria-label="Previous slide"
                    class="w-9 h-9 flex items-center justify-center rounded-sm bg-white/5 border border-white/15 text-white/65 hover:bg-white/15 hover:border-white/30 hover:text-white transition-all duration-200">
    ...
    <div id="heroThumbStrip"
         class="absolute right-6 top-1/2 -translate-y-1/2 w-20 z-20 flex-col gap-2 hidden lg:flex">
    </div>

    
    <div class="absolute bottom-0 left-0 right-0 h-[72px] z-30 flex items-center px-4 gap-3 lg:hidden">
        <div class="text-[10px] tracking-[0.1em] text-white/40 font-mono whitespace-nowrap shrink-0 w-[34px]" x-text="counter"></div>

        <button @click="prev" aria-label="Previous slide"
                class="w-8 h-8 shrink-0 flex items-center justify-center rounded-sm bg-white/5 border border-white/15 text-white/65 hover:bg-white/15 hover:text-white transition-all duration-200">
        ...
        <div id="heroThumbMobile" class="flex gap-2 items-center flex-1 overflow-hidden justify-center"></div>
    </div>

    
    <div x-show="loading" class="absolute inset-0 flex items-center justify-center bg-[#080808] z-50">
        <div class="w-10 h-10 border-2 border-white/20 border-t-white/70 rounded-full animate-spin"></div>
    </div>
</div>

<style>
    /* Slide states - Keeping CSS animations (can't be replaced by Tailwind) */
    .hs-slide {
        position: absolute;
        inset: 0;
        pointer-events: none;
    ...
    .hs-scrim {
        position: absolute;
        inset: 0;
        background: linear-gradient(105deg, rgba(5,3,12,0.95) 25%, rgba(5,3,12,0.55) 60%, rgba(5,3,12,0.15) 100%);
    }
    .hs-noise {
        position: absolute;
        inset: 0;
    ...
    @media (max-width: 1023px) {
        .hs-content { padding: 0 48px 100px 44px; }
    }
    @media (max-width: 640px) {
        .hs-content { padding: 0 24px 90px 24px; }
    }
    ...
    .hs-thumb.hs-ton .hs-thumb-bg { filter: brightness(0.7); }
    .hs-thumb:not(.hs-ton):hover .hs-thumb-bg { filter: brightness(0.55); transform: scale(1.05); }
    .hs-thumb-num {
        position: absolute;
        bottom: 6px;
        left: 0;
    ...
    .hs-thumb-m.hs-ton .hs-thumb-m-bg { filter: brightness(0.7); }
    .hs-thumb-m-num {
        position: absolute;
        bottom: 2px;
        left: 0;
    ...
    .hs-dot.hs-don { width: 28px; background: rgba(255,255,255,0.75); }
            ...
            this.slides.forEach((s, i) => {
                ...
                slide.innerHTML = `
                    ...
                    <div class="hs-content">
                        <div class="hs-tag"><span class="hs-tag-line"></span>${s.tag}</div>
                ...
                // Desktop thumbnail (smaller, centered)
...
</script><?php /**PATH C:\Users\User\Herd\ja-lanka-ecommerce\resources\views/partials/coverflow-hero.blade.php ENDPATH**/ ?>