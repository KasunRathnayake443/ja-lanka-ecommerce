...
class CheckoutController extends Controller
{
    ...
    public function index()
    {
        ...
        if ($cartItems->count() === 0) {
            return redirect()->route('mobile.shop')->with('error', 'Your cart is empty');
        }
        ...
        return view('mobile.checkout.index', compact('cartItems', 'subtotal', 'shipping', 'discount', 'total', 'addresses', 'coupon'));
    }
    ...
    public function placeOrder(Request $request)
    {
        ...
        if (!$checkoutData || empty($checkoutData['items'])) {
            return redirect()->route('mobile.cart')->with('error', 'Your cart is empty');
        }
        ...
        try {
            ...
            if ($request->payment_method === 'cod') {
                return redirect()->route('mobile.checkout.success', $order->id)
                    ->with('success', 'Order placed successfully!');
            }

            return redirect()->route('mobile.checkout.payment', $order->id);
    ...
    public function success($orderId)
    {
        ...
        return view('mobile.checkout.success', compact('order'));
    }

    public function payment($orderId)
    {
        $order = Order::where('user_id', Auth::id())->findOrFail($orderId);
        return view('mobile.checkout.payment', compact('order'));
    }
}