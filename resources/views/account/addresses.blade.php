...
@section('title', 'My Addresses - Ja Lanka')
                ...
                @if($addresses->count() > 0)
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-5">
                        ...
                        <div class="border border-gray-200 rounded-lg p-5 relative">
                            ...
                            <p class="text-sm text-gray-700">{{ $address->full_name }}</p>
                            ...
                            <div class="flex gap-3 mt-4 pt-3 border-t border-gray-100">
                                <button onclick="editAddress({{ json_encode($address) }})" class="text-blue-600 hover:text-blue-800 text-sm">Edit</button>
                                @if(!$address->is_default)
                                    <form action="{{ route('account.addresses.set-default', $address->id) }}" method="POST" class="inline">
                                        @csrf
                                        @method('PUT')
        ...
        <form id="addressForm" method="POST" class="p-6 space-y-5">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-5">
                ...
                <div>
                    ...
                    <input type="text" name="full_name" id="addressFullName" required class="w-full border border-gray-200 rounded-lg px-4 py-2 focus:border-red-500" value="{{ Auth::user()->name }}">
                </div>
                
                <div>
                    <label class="block text-sm font-medium text-gray-700 mb-1">Mobile Number *</label>
                    <input type="tel" name="mobile" id="addressMobile" required class="w-full border border-gray-200 rounded-lg px-4 py-2 focus:border-red-500" value="{{ Auth::user()->mobile }}">
                </div>
...
const storeUrl = "{{ route('account.addresses.store') }}";
const updateUrl = "{{ route('account.addresses.update', ['id' => 'ADDRESS_ID']) }}";
