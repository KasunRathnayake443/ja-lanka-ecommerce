...
@section('page_title', 'Categories & Brands')
            ...
            <table class="w-full">
                ...
                <tbody class="divide-y divide-gray-200">
                    ...
                    <tr>
                        ...
                        <td class="px-6 py-4">
                            <span class="px-2 py-1 text-xs rounded-full {{ $category->type == 'food' ? 'bg-green-100 text-green-800' : 'bg-blue-100 text-blue-800' }}">
                        ...
                        <td class="px-6 py-4 text-right">
                            ...
                            <form action="{{ route('admin.store.category.delete', $category->id) }}" method="POST" class="inline">
                                @csrf
                                @method('DELETE')
            ...
            <table class="w-full">
                ...
                <tbody class="divide-y divide-gray-200">
                    ...
                    <tr>
                        ...
                        <td class="px-6 py-4">{{ $brand->country ?? '-' }}</td>
                        <td class="px-6 py-4">
                        ...
                        <td class="px-6 py-4 text-right">
                            <button onclick='editBrand({{ json_encode($brand) }})' class="text-blue-600 hover:text-blue-900 mr-3">Edit</button>
                            <form action="{{ route('admin.store.brand.delete', $brand->id) }}" method="POST" class="inline">
                                @csrf
                                @method('DELETE')
        ...
        <div class="overflow-x-auto">
            <table class="w-full">
                ...
                <tbody class="divide-y divide-gray-200">
                    ...
                    <tr>
                        <td class="px-6 py-4 text-xl">{{ $origin->flag_icon ?? '🌏' }}</td>
                        <td class="px-6 py-4">{{ $origin->country_name }}</td>
                        <td class="px-6 py-4">
                            <span class="{{ $origin->is_active ? 'text-green-600' : 'text-red-600' }}">
                                {{ $origin->is_active ? 'Active' : 'Inactive' }}
                            </span>
                        </td>
                        <td class="px-6 py-4 text-right">
                            <button onclick='editOrigin({{ json_encode($origin) }})' class="text-blue-600 hover:text-blue-900 mr-3">Edit</button>
                            <form action="{{ route('admin.store.origin.delete', $origin->id) }}" method="POST" class="inline">
                                @csrf
                                @method('DELETE')
...
<script>
// Route URLs
const categoryStoreUrl = "{{ route('admin.store.category.store') }}";
const categoryUpdateUrl = "{{ route('admin.store.category.update', ['id' => '__ID__']) }}";
const brandStoreUrl = "{{ route('admin.store.brand.store') }}";
const brandUpdateUrl = "{{ route('admin.store.brand.update', ['id' => '__ID__']) }}";
const originStoreUrl = "{{ route('admin.store.origin.store') }}";
const originUpdateUrl = "{{ route('admin.store.origin.update', ['id' => '__ID__']) }}";
