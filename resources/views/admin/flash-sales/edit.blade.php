...
@section('page_title', 'Edit Flash Sale Banner')
...
<div class="bg-white rounded-xl shadow-md overflow-hidden">
    ...
    <form action="{{ route('admin.flash-sales.update', $banner->id) }}" method="POST">
        @csrf
        @method('PUT')
        ...
        <div class="p-6 space-y-6">
            ...
            <div class="bg-gray-50 rounded-lg p-4">
                ...
                <div class="flex gap-4">
                    @if($banner->product && $banner->product->images->first())
                        <img src="{{ asset('storage/' . $banner->product->images->first()->image_path) }}" 
                    ...
                    <div class="flex-1">
                        <div class="font-semibold text-gray-800">{{ $banner->product->name ?? 'Product Deleted' }}</div>
                        <div class="text-sm text-gray-500">SKU: {{ $banner->product->sku ?? 'N/A' }}</div>
                        
                        @if($banner->product && $banner->product->hasActiveSale())
                            <div class="mt-2">
                                <span class="text-red-600 font-bold">LKR {{ number_format($banner->product->sale_price, 2) }}</span>
                                <span class="text-gray-400 line-through ml-2">LKR {{ number_format($banner->product->regular_price, 2) }}</span>
                                <span class="ml-2 px-2 py-0.5 bg-red-100 text-red-800 rounded text-xs">
                                    -{{ $banner->product->discount_percent }}% OFF
                                </span>
                            </div>
                        @elseif($banner->product)
                            <div class="mt-2">
                                <span class="text-gray-800">LKR {{ number_format($banner->product->regular_price, 2) }}</span>
            ...
            <div>
                ...
                <input type="text" name="custom_title" value="{{ $banner->custom_title }}" 
            ...
            <div>
                ...
                <input type="text" name="custom_subtitle" value="{{ $banner->custom_subtitle }}" 
                        ...
                        <div class="flex">
                            ...
                            <div class="flex-1 p-3">
                                <div class="font-semibold text-sm">
                                    {{ $banner->custom_title ?? ($banner->product->name ?? 'Product Name') }}
                                </div>
                                @if($banner->custom_subtitle || ($banner->product && $banner->product->hasActiveSale()))
                                    <div class="text-xs text-gray-500 mt-1">
                                        {{ $banner->custom_subtitle ?? ($banner->product && $banner->product->hasActiveSale() ? 'Limited time offer!' : '') }}
                                    </div>
                                @endif
                                @if($banner->product && $banner->product->hasActiveSale())
                                    <div class="mt-2">
                                        <span class="text-red-600 font-bold text-sm">LKR {{ number_format($banner->product->sale_price, 2) }}</span>
                                        <span class="text-gray-400 text-xs line-through ml-1">LKR {{ number_format($banner->product->regular_price, 2) }}</span>
                                    </div>
                                @elseif($banner->product)
                                    <div class="mt-2">
                                        <span class="text-gray-800 text-sm">LKR {{ number_format($banner->product->regular_price, 2) }}</span>
                                    </div>
                                @endif
